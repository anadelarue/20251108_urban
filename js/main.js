...
mobileFiltersBtn.addEventListener('click', () => {
    ...
    mobileFiltersBtn.innerHTML = 
        mobileFiltersBtn.innerHTML === 'Abrir filtros' 
        ? 'Cerrar filtros' 
        : 'Abrir filtros';
});
...
let zoom = mapWidth > 525 ? 12 : 13;
let minZoom = mapWidth > 525 ? 11 : 10.5;
let maxZoom = 18;
...
function initMap() {
    ...
    map = new mapboxgl.Map({
        ...
        minZoom: minZoom,
        maxZoom: maxZoom,
        center: [-3.687490780590369, 40.43332390784789]
    });
    ...
    map.addControl(
        new MapboxGeocoder({
            ...
            countries: 'es',
            marker: false,
            placeholder: 'Busca tu calle o municipio'
        })
    );
    ...
    //Una vez cargados los datos externos, cargamos el mapa
        map.on('load', function(){
            ...
            map.addSource('distritos', {
                'type': 'vector',
                'url': 'mapbox://anadelarue.cq8m050d'
            });
            ...
            layersObject.forEach(layer =>{
                ...
                'type': 'fill',
                paint: {
                    // color categories using a match expression
                    'fill-color': [
                    'match',
                    ['get', 'currentUse'], // the property name in your data
                    '1_residential', '#87a6bd',
                    '2_agriculture', '#ba7021',
                    '3_industrial', '#757d2b',
                    '4_1_office', '#ebab36',
                    '4_2_retail', '#821c28',
                    '4_3_publicServices', '#f0aba6',
                    /* other */ '#CCCCCC'
                    ],
                    'fill-opacity': 1
                }           
            ...
            layersObject.forEach(layer =>{
                ...
                paint: {
                    ...
                    'line-width': [
                        ...
                        ['linear'],
                        ['zoom'],
                        11,  0.005,   // Zoom 5 → grosor 0.5
                        12, 0.01,     // Zoom 10 → grosor 2
                        15, 0.5
                    ]
            ...
            map.addLayer({
                'id': 'distritos',
                'source': 'distritos',
                'source-layer': 'secciones_merge-claosq',
                'type': 'fill',
                paint: {
                    // color categories using a match expression
                    'fill-color': '#2f52a0',
            ...
            mapCheckBoxesAll.forEach(mapCheckBoxes => {
                mapCheckBoxes.addEventListener('click', (ev) => {
                    let useTarget = ev.target.className;

                    const useTargetObject = checkBoxType.find(useTargetItem => useTargetItem.type == useTarget);

                    useTargetObject.visible = useTargetObject.visible == true ? false : true;

                    const visibleTypes = checkBoxType
                        .filter(item => item.visible)
                        .map(item => item.type);

                    const layersList = layersObject.map(item => item.id);

                    layersList.forEach(layer => {
                        map.setPaintProperty(layer, 'fill-opacity', [
                            'case',
                            ['in', ['get', 'currentUse'], ['literal', visibleTypes]],
                            1, // visible
                            0    // oculto
                            ]);
                        });
        ...
        map.on('mousemove', function (e) {
          ...
          if (map.getZoom() >= 14) {
            var municipiosFeatures = map.queryRenderedFeatures(e.point, { layers: ['distritos'] });
            var edificiosLayers = ['antes-40', '40-60', '60-80', '80-00', 'desde-2000']; // Agrega las nuevas capas aquí
            ...
            if (municipiosFeatures.length > 0 && edificiosFeatures.length > 0) {
              ...
              municipiosFeatures.forEach(function (feature) {
                tooltipText += `<p style = 'font-family:Roboto;font-weight:700;font-size:12px;'>${feature.properties.NMUN} (${feature.properties.NPRO})</p>`;
                tooltipText += `<p style = 'font-family:Roboto;font-size:12px; opacity: 0.8; margin-bottom: 5px;'>Distrito ${feature.properties.CDIS}</p>`;
                tooltipText += `<div style = ''>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #87a6bd;'></span>
                                        <p><span style = 'font-weight: 600;'>Residencia:</span> ${feature.properties['1_residential'] ? feature.properties['1_residential'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #ba7021;'></span>
                                        <p><span style = 'font-weight: 600;'>Agricultura:</span> ${feature.properties['2_agriculture'] ?  feature.properties['2_agriculture'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #757d2b;'></span>
                                        <p><span style = 'font-weight: 600;'>Industria:</span> ${feature.properties['3_industrial'] ? feature.properties['3_industrial'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #ebab36;'></span>
                                        <p><span style = 'font-weight: 600;'>Oficina:</span> ${feature.properties['4_1_office'] ? feature.properties['4_1_office'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #821c28;'></span>
                                        <p><span style = 'font-weight: 600;'>Comercio:</span> ${feature.properties['4_2_retail'] ? feature.properties['4_2_retail'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
                                    <div style = 'display:flex; gap: 8px; align-items:center;'>
                                        <span style = 'display: block; width: 12px; height: 12px; background-color: #f0aba6;'></span>
                                        <p><span style = 'font-weight: 600;'>Edificio público:</span> ${feature.properties['4_3_publicServices'] ? feature.properties['4_3_publicServices'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : 0}</p>
                                    </div>
